...
import '../phase2/location_page.dart';
...
class _LoginPageState extends State<LoginPage> with Validator{
  ...
  _getUser() async {
    if (_formkey.currentState.validate()){
      ...
      pr.show();
      print("Connection.login URL========== ${Connection.validateLogin}");
      print("name.text ========== ${name.text}");
      print("pass.text========== ${pass.text}");
      var response = await http.post(Uri.parse(Connection.validateLogin), body: {
      'LoginID':name.text,
      'Password':pass.text,
        'UserName':name.text,
      });
      ...
      if (results['UserID'] != null) {
        ...
        print("UserID =============>  ${results['UserID'].toString()}");
        print("object role $role");
        globals.Role = role;


        Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => LocationPage()), (Route<dynamic> route) => false);
        // if(role == "Admin"){
